...
const ImpactCarousel: React.FC<ImpactCarouselProps> = ({ schoolImages, slider }) => {
    ...
    return (
        <>
            ...
            >
                <CarouselContent>
                    {slider === "comunitySlider" && allBannerImg.map((image, index) => (
                        <CarouselItem key={index} className="basis-1/2 md:basis-1/3 ">
                            <Image src={image} width={800} height={500} className='w-full' alt='banner'></Image>
                        </CarouselItem>
                    ))}
                    {slider === "schoolSlider" && schoolImages.map((image, index) => (
                        <CarouselItem key={index} className="basis-1/2 md:basis-1/3 lg:basis-1/4 pl-4 bg-white">
                            <div className=''>
                                <Image src={image} width={800} height={500} className='w-full rounded-md ' alt='banner'></Image>
                                <p className='font-semibold pt-3'>{data[index].name}</p>
                                <div className='flex gap-1 items-center'>
                                    <IoLocation className='text-baseBg' />
                                    <p className='text-sm text-gray-500'>{data[index].location}</p>
                                </div>
                            </div>
                        </CarouselItem>
                    ))}
